...
export const PATCH = withRBAC(
  async (req, { params }) => {
    try {
      ...
      const result = await ApprovalService.resolveApproval(id, "REJECTED", req.user.id, parsed.data.remarks);
      return NextResponse.json({ data: result });
    } catch (error: any) {
      return NextResponse.json({ error: error.message || "Failed to reject quotation" }, { status: 500 });
    }
  },
  [Role.ADMIN, Role.MANAGER] // Only Managers and Admins can reject
);
