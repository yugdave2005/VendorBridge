...
export const PATCH = withRBAC(
  async (req, { params }) => {
    try {
      ...
      const updatedRfq = await RFQService.updateStatus(id, RFQStatus.CLOSED, req.user.id);
      return NextResponse.json({ data: updatedRfq });
    } catch (error: any) {
      return NextResponse.json({ error: error.message || "Failed to close RFQ" }, { status: 500 });
    }
  },
